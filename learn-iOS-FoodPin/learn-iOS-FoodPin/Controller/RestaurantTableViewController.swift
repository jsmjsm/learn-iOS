...
//			(actin: UIAlertAction!) -> Void in
//			let alterMessage = UIAlertController(title: "Service Unavaliable", message: "Sorry the call feature si not avaliable yet. Please retry later.", preferredStyle: .alert)
//			
    ...
    */
	...
	override func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
		...
		let deleteAction = UIContextualAction(style: .destructive, title: "Delete") { (action, sourceView, completionHandler) in
			...
			self.tableView.deleteRows(at: [indexPath], with: .fade) //从图标中删除
		...
		let shareAction = UIContextualAction(style: .normal, title: "Share") {(action, sourceView, completionHandler) in
			let defaultText = "Just checking in at " + self.restaurants[indexPath.row].name // 设置分享的文字
			// 分享控制器
	...
	override func tableView(_ tableView: UITableView, leadingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
		...
		// 签到按钮的设计
		checkInAction.backgroundColor = #colorLiteral(red: 0.1415770948, green: 0.6326834559, blue: 0.3052188754, alpha: 1)
